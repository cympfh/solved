...
fn main() {
    ...
    // dp[i][c] := 頭 i 個決めて, 最後が c 以下で終わるモノの数
    let mut dp = ndarray![mint!(0); n + 1, 3010];
    for z in 0..3010 {
        dp[0][z] = mint!(1);
    }
    for i in 0..n {
        ...
        {
            let dg = &dp[i + 1][0..20].iter().map(|m| m.0).collect::<Vec<_>>();
            trace!(i + 1, dg);
        }
    }
    let ans: ModInt = dp[n][3000];
    put!(ans);
}
