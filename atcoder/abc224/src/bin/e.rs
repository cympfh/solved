...
fn main() {
    ...
    rs.sort_by_key(|&(_, _, a, _)| Reverse(a));

    let mut ans = vec![0; n];

    use Hyper::*;
    let mut row_max = vec![NegInf; h + 2];
    let mut col_max = vec![NegInf; w + 2];
    let mut updates = vec![];
    let mut last_a = 2_000_000_000;
    for (r, c, a, idx) in rs {
        trace!((r, c, a), idx);
        if last_a > a && !updates.is_empty() {
            for &(r, c, z) in updates.iter() {
                row_max[r] = max!(row_max[r], z);
                col_max[c] = max!(col_max[c], z);
            }
            updates.clear();
            trace!("!", &row_max);
            trace!("@", &col_max);
        }
        let z = max!(row_max[r] + 1, col_max[c] + 1, Real(0));
        trace!(z);
        updates.push((r, c, z));
        ans[idx] = z.unwrap();
        last_a = a;
