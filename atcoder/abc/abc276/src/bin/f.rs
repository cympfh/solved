...
fn main() {
    ...
    let mut bit_count: BIT<i64> = BIT::new(200_100);
    let mut bit_sum: BIT<ModInt> = BIT::new(200_100);

    let mut e = mint!(a[0] as i64);
    put!(e);
    ...
    for i in 1..n {
        trace!(i, a[i]);
        let an = mint!(a[i] as i64);
        let m = bit_count.sum(0..a[i]);
        let asum = bit_sum.sum(a[i]..200_100);

        let k = mint!((i + 1) as i64);
        e = (k - 1).pow(2) * e + an + 2 * (an * m + asum);
        e = e / k / k;
        put!(e);
