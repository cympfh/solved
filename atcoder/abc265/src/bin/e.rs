...
fn main() {
    ...
    let n: usize = sc.cin();
    let m: usize = sc.cin();
    let (a, b): (i64, i64) = sc.pair();
    let (c, d): (i64, i64) = sc.pair();
    let (e, f): (i64, i64) = sc.pair();
    let obs: Vec<(i64, i64)> = (0..m).map(|_| sc.pair()).collect();

    let mut dp = DefaultDict::new(mint!(0));
    dp[(0_i64, 0_i64)] = mint!(1);

    for _ in 0..n {
        let mut alt = DefaultDict::new(mint!(0));
        for ((x, y), &cx) in dp.iter() {
            if !obs.contains(&(x + a, y + b)) {
                alt[(x + a, y + b)] += cx;
            }
            if !obs.contains(&(x + c, y + d)) {
                alt[(x + c, y + d)] += cx;
            }
            if !obs.contains(&(x + e, y + f)) {
                alt[(x + e, y + f)] += cx;
            }
        }
        dp = alt;
