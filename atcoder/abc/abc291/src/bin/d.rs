...
fn main() {
    ...
    // dp[i][j] = ( cs[0..i] について, j=裏向けた )
    let mut dp = ndarray![ mint!(0) ; n + 1, 2];
    dp[1][0] = mint!(1);
    dp[1][1] = mint!(1);
    let zero = mint!(0);
    for i in 1..n {
        dp[i + 1][0] = iff!(cs[i - 1].0 != cs[i].0, dp[i][0], zero)
            + iff!(cs[i - 1].1 != cs[i].0, dp[i][1], zero);
        dp[i + 1][1] = iff!(cs[i - 1].0 != cs[i].1, dp[i][0], zero)
            + iff!(cs[i - 1].1 != cs[i].1, dp[i][1], zero);
    }
    put!(dp[n][0] + dp[n][1]);
}
