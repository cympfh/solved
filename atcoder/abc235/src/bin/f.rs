...
use std::{cmp::*, collections::*, ops::RangeBounds};

fn main() {
    let mut sc = Scanner::new();
    let digits: Vec<usize> = {
        let n: Vec<char> = sc.chars();
        n.iter()
    ...
    // dp_c[iset] = #( iset を使って N より真に小さい )
    let mut dp_c = ndarray![mint!(0); 1024];
    // その和
    let mut dp_s = ndarray![0_i64; 1024];

    let mut fset = 1 << digits[0];
    let mut full = mint!(digits[0] as i64);
    for d in 1..digits[0] {
        dp_c[1 << d] += 1;
        dp_s[1 << d] += d as i64;
    }
    ...
    for i in 1..digits.len() {
        let mut alt_c = ndarray![mint!(0); 1024];
        let mut alt_s = ndarray![0_i64; 1024];
        for d in 1..10 {
            alt_c[1 << d] += 1;
            alt_s[1 << d] += d as i64;
        }
        for d in 0..10 {
            for iset in 0..1024 {
                let jset = iset | (1 << d);
                alt_c[jset] += dp_c[iset];
                alt_s[jset] += dp_s[iset] * 10 + dp_c[iset].0 * d as i64;
            }
        }
        for d in 0..digits[i] {
            let jset = fset | (1 << d);
            alt_c[jset] += 1;
            alt_s[jset] += full.0 * 10 + d as i64;
        }
        dp_c = alt_c;
        for iset in 0..1024 {
            dp_s[iset] = alt_s[iset] % MOD_998244353;
        }
        fset |= 1 << digits[i];
        full = full * 10 + digits[i] as i64;
    }
