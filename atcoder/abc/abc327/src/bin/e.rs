...
fn main() {
    ...
    let n: usize = sc.cin();
    let p: Vec<f64> = sc.vec(n);

    use Hyper::*;

    // S(Q) = Sum[ gamma.pow(i) * Q[i] ]
    // Z = S([1,1, ..., 1])
    // R = S(Q) / Z - 1200/k.sqrt()
    // dp[i][k] == (n[0..=i] からちょうど k 個選んで出来る S(Q)  の最大値
    let mut dp = ndarray![NegInf; n, n+1];
    dp[0][1] = Real(p[0]);

    for i in 1..n {
        dp[i][1] = max!(dp[i - 1][1], Real(p[i]));
        for k in 2..=n {
            if let Real(x) = dp[i - 1][k - 1] {
                dp[i][k] = max!(dp[i - 1][k], Real(x * 0.9 + p[i]));
            } else {
                dp[i][k] = dp[i - 1][k];
            }
        }
    ...
    for i in 0..n {
        for k in 1..=n {
            if let Real(x) = dp[i][k] {
                ans = max!(ans, Real(x / z[k] - 1200.0 / (k as f64).sqrt()));
            }
        }
    }
    put!(ans.unwrap());
