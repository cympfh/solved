...
fn main() {
    ...
    let ts: HashSet<String> = sc.vec(m).into_iter().collect();
    ...
    for t in ts.iter() {
        ...
        for i in 1..n {
            ...
            prefixes.insert(t[..i].to_string());
        }
        prefixes.insert(t.clone());
    }
    trace!(&prefixes);

    for p in Permutation::new(n) {
        let mut c = ss[p[0]].clone();
        for i in 1..n {
            c.push('_');
            while prefixes.contains(&c) {
                c.push('_');
                if c.len() > 17 {
                    c.push('@');
                    c.push('@');
            ...
            c.push_str(ss[p[i]].as_str());
        }
        trace!(&p, &c);
        if 3 <= c.len() && c.len() <= 16 && !ts.contains(&c) {
            put!(c);
            return;
        }
    }
