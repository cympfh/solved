...
fn main() {
    ...
    // d[k] = ビット (1 << k) を最上位ビットとして立てられる M 以下の個数
    let mut d = ndarray![0_u64; 61];
    for k in 0..60 {
        d[k] = if (1 << k) > m {
            0
        } else if (1_u64 << (k + 1)) - 1 <= m {
            1 << k
        } else {
            m - (1 << k) + 1
        };
    ...
    trace!(&d);
    let d: Vec<ModInt> = d.into_iter().map(|z| mint!(z as i64)).collect();

    let mut dp: Vec<ModInt> = d.clone();
    for _ in 1..n {
        let cum = Cumsum1d::new(&dp);
        let mut alt = vec![mint!(0); 61];
        for k in 1..60 {
            alt[k] = d[k] * cum.sum(0..k);
        }
        dp = alt;
        trace!(&dp);
    }
    let ans: ModInt = dp.iter().cloned().sum();
    put!(ans);
}
