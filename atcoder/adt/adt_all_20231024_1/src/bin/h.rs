...
fn main() {
    ...
    let mut memo: HashMap<_, usize> = HashMap::new();

    for s in ss.iter() {
        let mut sh = StringHash::new();
        for &c in s.iter() {
            sh.push_back(c);
            let d = sh.dump();
            let x = memo.get(&d);
    ...
    for s in ss.iter() {
        ...
        for &c in s.iter() {
            sh.push_back(c);
            let num = match memo.get(&sh.dump()) {
                None => 0,
                Some(&c) => c,
            };
            if num <= 1 {
...
impl StringHash {
    pub fn dump(&self) -> (i128, i128) {
        let x = self.sethash.0;
        let y = self.rolhash.data.0;
        (x, y)
    }
    pub fn new() -> Self {
        let length = 0;
