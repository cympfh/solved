...
fn main() {
    ...
    for _ in 0..t {
        ...
        fn state(u: usize, v: usize, n: usize) -> usize {
            u * n + v
        }
        fn destate(s: usize, n: usize) -> (usize, usize) {
            (s / n, s % n)
        }
        ...
        memo[state(0, n - 1, n)] = Real(0);
        que.push_back((Reverse(Real(0)), state(0, n - 1, n)));
        while let Some((Reverse(d), s)) = que.pop_front() {
            let (u, v) = destate(s, n);
            trace!(u, v);
            if used[s] {
                continue;
            ...
            for &u2 in g[u].iter() {
                for &v2 in g[v].iter() {
                    ...
                    let s2 = state(u2, v2, n);
                    if memo[s2] > d + 1 {
                        memo[s2] = d + 1;
                        que.push_back((Reverse(d + 1), s2));
                    }
                }
            }
        }
        let goal = state(n - 1, 0, n);
        if let Real(ans) = memo[goal] {
            put!(ans);
        } else {
            put!(-1);
        }
