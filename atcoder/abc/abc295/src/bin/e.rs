...
fn main() {
    ...
    xs.sort();
    let num_zero = xs.iter().filter(|&&x| x == 0).count();

    let mut ans = mint!(0);
    for b in 1..=m {
        // b 以上が n-k+1 個以上になる確率
        let mut pr = mint!(0);
        let r = xs.iter().filter(|&&x| x >= b).count(); // すでに m 個ある

        // 一つが b 以上になる確率
        let p1 = mint!((m - b + 1) as i64) / mint!(m as i64);
        // 一つが b 未満になる確率
        let p2 = mint!((b - 1) as i64) / mint!(m as i64);
        trace!(p1, p2);

        // memo
        let mut u = binom!(num_zero as u64, 0);

        // 今 0 なものの内 t 個が b 以上になった
        for t in 0..=num_zero {
            if r + t < n - k + 1 {
                continue;
            }
            u = Binom::new_with_hint(num_zero as u64, t as u64, &u);
            pr += u.unwrap() * p1.pow(t as i64) * p2.pow((num_zero - t) as i64);
            trace!(t, u, p1, p2);
        }
        trace!(b, pr);
        ans += pr;
