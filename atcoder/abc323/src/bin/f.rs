...
fn between(x: i128, sub: i128, sup: i128) -> bool {
    if sub <= sup {
        sub <= x && x <= sup
    } else {
        sup <= x && x <= sub
...
fn dist(p: P, q: P, z: P) -> i128 {
    let dx = (p.0 - q.0).abs();
    let dy = (p.1 - q.1).abs();
    if dx == 0 && dy == 0 {
        0
    } else if dx == 0 && p.0 == z.0 && between(z.1, p.1, q.1) {
        dy + 2
    } else if dy == 0 && p.1 == z.1 && between(z.0, p.0, q.0) {
        dx + 2
    } else {
        dx + dy
    }
...
fn main() {
    assert_eq!(dist(P(0, 0), P(1, 0), P(2, 0)), 1);
    ...
    use Hyper::*;
    let mut ans = Inf;
    let dirs = vec![P(1, 0), P(-1, 0), P(0, 1), P(0, -1)];

    for &d in dirs.iter() {
        trace!(#plan1, d, plan1(a, b, c, d));
        ans = min!(ans, plan1(a, b, c, d));
    }
    for &d1 in dirs.iter() {
        for &d2 in dirs.iter() {
            trace!(#plan2, d1,d2, plan2(a, b, c, d1,d2));
            ans = min!(ans, plan2(a, b, c, d1, d2));
        }
    }
...
fn plan2(a: P, b: P, c: P, dir1: P, dir2: P) -> Hyper<i128> {
    use Hyper::*;
    let byoko = sub(b, dir1);
    let k = div1(sub(c, b), dir1);
    // trace!(#plan2 (dir1, dir2), (byoko, k));
    if k >= 0 {
        let a_new = add(byoko, mul(dir1, k));
        let b_new = add(b, mul(dir1, k));
        if let Real(cost) = plan1(a_new, b_new, c, dir2) {
            return Real(cost + dist(a, byoko, b) + k);
        }
    }
    Inf
}

fn plan1(a: P, b: P, c: P, pushdir: P) -> Hyper<i128> {
    let byoko = sub(b, pushdir);
    let dist1 = dist(a, byoko, b);
    let k = div1(sub(c, b), pushdir);
    if k >= 0 {
        let b = add(b, mul(pushdir, k));
        if b == c {
            return Hyper::Real(dist1 + k);
        }
    }
    Hyper::Inf
}
