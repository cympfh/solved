...
fn main() {
    ...
    dp[vec![0_usize; k]] = Real(0_i64);

    for _ in 0..n {
        let cost: i64 = sc.cin();
        let dx: Vec<usize> = sc.vec(k);

        let mut alt = dp.clone();
        for (state, val) in dp.iter() {
            let t = add(state, &dx, p);
            let x = alt[t.clone()];
            alt[t] = min!(x, *val + Real(cost));
        }
        dp = alt;
    }

    if let Real(ans) = dp[vec![p; k]] {
        put!(ans);
    } else {
        put!(-1);
    }
...
fn add(s: &Vec<usize>, t: &Vec<usize>, p: usize) -> Vec<usize> {
    let mut r = t.clone();
    for i in 0..t.len() {
        r[i] += s[i];
        if r[i] > p {
