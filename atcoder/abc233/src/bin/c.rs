...
fn main() {
    ...
    let n: usize = sc.cin();
    let goal: u128 = sc.cin();
    let mut memo = DefaultDict::new(0);
    memo[1] = 1;
    for _ in 0..n {
        let size: usize = sc.cin();
        let xs: Vec<u128> = sc.vec(size);
        let mut alt = DefaultDict::new(0);
        for (factor, count) in memo.iter() {
            for &x in xs.iter() {
                let z = x * factor;
                if z <= goal && goal % z == 0 {
                    alt[z] += count;
