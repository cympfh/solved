...
fn nextindex(x: usize, set: &BTreeSet<usize>) -> Option<usize> {
    if let Some(&i) = set.range(x..).next() {
        if i < M {
            return Some(i);
        }
    }
...
fn main() {
    ...
    let mut a = DefaultDict::new(-1_i64);
    for _ in 0..q {
        let ty: usize = sc.cin();
        let x: usize = sc.cin::<usize>();
        if ty == 1 {
            trace!(x, nextindex(x % M, &set));
            if let Some(i) = nextindex(x % M, &set) {
                a[i] = x as i64;
                set.remove(&i);
            } else if let Some(i) = nextindex(0, &set) {
                a[i] = x as i64;
                set.remove(&i);
            }
            trace!(&a);
