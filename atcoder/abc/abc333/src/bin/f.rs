...
fn main() {
    ...
    // p[n][m] = n 人いたときに m 番目が最初に消える確率
    let mut p = ndarray![mint!(0); 3001, 3001];
    for n in 1..=3000_usize {
        let a = mint!(1) / (mint!(1) - half[n]);
        for m in 1..=n {
            p[n][m] = a * half[m];
        }
    }

    // q[n][m] = m 番目が最後まで生き残る確率
    let mut q = ndarray![mint!(0); 3001, 3001];
    q[1][1] = mint!(1);
    for n in 2..=nn {
        let pbit = BIT::from(&p[n]);
        for m in 1..=n {
            q[n][m] = pbit.sum(1..m) * q[n - 1][m - 1] + pbit.sum(m + 1..n + 1) * q[n - 1][m];
        }
        trace!(n, &q[n][1..=n]);
    }
