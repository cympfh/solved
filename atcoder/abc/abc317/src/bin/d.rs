...
fn main() {
    ...
    // dp[n] = (n議席得るのに必要な鞍替えの最小数)
    let mut dp = vec![Inf; 100_100];
    {
        let mut sum = 0;
    ...
    for &(x, y, z) in xyz.iter() {
        ...
        let dx = (x + y + 1) / 2 - x;
        for n in (0..100_100 - z).rev() {
            dp[n + z] = min!(dp[n + z], dp[n] + dx);
        }
        trace!(&dp[0..20]);
    }

    let zsum: usize = xyz.iter().map(|tup| tup.2).sum();
    trace!(zsum, (zsum + 1) / 2);
    let ans = ((zsum + 1) / 2..100_100).map(|z| dp[z]).min().unwrap();
    put!(ans.unwrap());
}
