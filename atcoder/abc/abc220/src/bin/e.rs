...
fn main() {
    ...
    let n: i64 = sc.cin();
    let depth = n - 1;
    let distance: i64 = sc.cin();
    let mut ans = mint!(0);
    {
        for d in 0..=depth {
            let nodes = mint!(2).pow(d);
            trace!(d, nodes);
            if d + distance <= depth {
                ans += nodes * mint!(2).pow(distance);
                trace!("v", mint!(2).pow(distance));
            }
            let z = depth - d;
            let way = if distance <= z + 1 {
                distance - 1
            } else if distance <= 2 * z {
                2 * z - distance + 1
            } else {
                0
            };
            trace!("w", nodes, way, mint!(2).pow(distance - 2));
            ans += nodes * way * mint!(2).pow(distance - 2);
        }
    }
    put!(ans * 2);
}
