...
fn coodinate_compression<T: Clone + Ord>(xs: &Vec<T>) -> Vec<usize> {
    let xset: BTreeSet<T> = xs.iter().cloned().collect();
    let xmap: BTreeMap<&T, usize> = xset.iter().enumerate().map(|(i, x)| (x, i)).collect();
...
fn main() {
    ...
    trace!(&b);
    let b = coodinate_compression(&b);
    trace!(&b);
    let mut ps: Vec<(u64, usize)> = a.into_iter().zip(b.into_iter()).collect();
    ps.sort_by_key(|&(a, b)| (a, Reverse(b)));
    trace!(&ps);

    let mut ans = 0_i64;
    let mut z = BIT::new(n + 10);
    let mut last_pair = (123, 456);
