...
fn main() {
    ...
    let a: Vec<usize> = coodinate_compression(&a);
    let mut abit: BIT<ModInt> = BIT::new(300_100);

    let mut ans = mint!(0);
    for i in (0..n).rev() {
        let x = abit.sum(a[i]..300_100);
        ans += x / mint!(2).pow(i as i64) / 2;
        let key = mint!(2).pow(i as i64);
        abit.add(a[i], key);
        trace!(a[i], key);
    }
    put!(ans);
