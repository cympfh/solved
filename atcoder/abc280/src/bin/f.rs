...
fn main() {
    ...
    let es: Vec<(usize, usize, i64)> = (0..m)
        .map(|_| {
            let a = sc.usize1();
            let b = sc.usize1();
            let c: i64 = sc.cin();
    ...
    let mut cgroup = UnionFind::new(n);

    use Hyper::*;
    let mut potential = vec![Real(0_i64); n];
    ...
    for root in 0..n {
        ...
        potential[root] = Real(0);
        while let Some(u) = stack.pop() {
            connected.insert(u);
            for &(v, cost) in g[u].iter() {
                if used[v] {
                    let pv = potential[u] + cost;
                    trace!((u, v), potential[u], cost, potential[v]);
                    if potential[v] != pv {
                        inf = true;
                    }
                } else {
    ...
    for _ in 0..q {
        let x = sc.usize1();
        let y = sc.usize1();
        trace!((x, y), cgroup.is_same(x, y), potential[x], potential[y]);
        trace!(potential[y] - potential[x]);
        if cgroup.is_same(x, y) {
            let dp = potential[y] - potential[x];
            if let Real(ans) = dp {
                put!(ans);
            } else {
                put!(#inf);
