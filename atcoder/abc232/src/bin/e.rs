...
fn main() {
    ...
    let w: i64 = sc.cin();
    let k: usize = sc.cin();
    let (x1, y1): (u64, u64) = sc.pair();
    let (x2, y2): (u64, u64) = sc.pair();

    let mut dp = ndarray![mint!(0); 2, 2];
    dp[0][0] += 1;
    for _ in 0..k {
        let mut alt = ndarray![mint!(0); 2, 2];
        alt[0][0] = dp[0][1] * (w - 1) + dp[1][0] * (h - 1);
        alt[0][1] = dp[0][0] + dp[1][1] * (h - 1);
        alt[0][1] += dp[0][1] * (w - 2);
        alt[1][0] = dp[0][0] + dp[1][1] * (w - 1);
        alt[1][0] += dp[1][0] * (h - 2);
        alt[1][1] = dp[0][1] + dp[1][0];
        alt[1][1] += dp[1][1] * (h - 2 + w - 2);
        dp = alt;
        // for i in 0..2 { trace!(&dp[i]); }
    }

    let i = if x1 == x2 { 0 } else { 1 };
    let j = if y1 == y2 { 0 } else { 1 };
    put!(dp[i][j]);
}
