...
impl Set {
    fn insert(&mut self, item: u64) {
        ...
        self.hash_num[0] += if item % 2 == 0 { 1 } else { 0 };
...
fn main() {
    ...
    let mut aset = Set::default();
    let mut bset = Set::default();
    ...
    ahash.push(aset.hash());
    bhash.push(bset.hash());
    for i in 0..n {
        aset.insert(a[i]);
        bset.insert(b[i]);
        ahash.push(aset.hash());
    ...
    for &(x, y) in qs.iter() {
        trace!((x, y), (ahash[x], bhash[y]));
        put!(if ahash[x] == bhash[y] { "Yes" } else { "No" });
    }
}
