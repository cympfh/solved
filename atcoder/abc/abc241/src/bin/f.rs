...
fn main() {
    ...
    let s = (sc.usize1(), sc.usize1());
    let t = (sc.usize1(), sc.usize1());
    let stones: Vec<_> = (0..n).map(|_| (sc.usize1(), sc.usize1())).collect();

    let mut rows = DefaultDict::new(BTreeSet::new());
    let mut cols = DefaultDict::new(BTreeSet::new());
    for &(x, y) in stones.iter() {
        rows[x].insert(y);
        cols[y].insert(x);
    }
    ...
    while let Some(u) = stack.pop() {
        let (x, y) = u;
        if let Some(y2) = rows[x].range(..y).next_back() {
            let v = (x, y2 + 1);
            if memo[v] > memo[u] + 1 {
                memo[v] = memo[u] + 1;
                stack.push(v);
        ...
        if let Some(y2) = rows[x].range(y + 1..).next() {
            let v = (x, y2 - 1);
            if memo[v] > memo[u] + 1 {
                memo[v] = memo[u] + 1;
                stack.push(v);
        ...
        if let Some(x2) = cols[y].range(..x).next_back() {
            let v = (x2 + 1, y);
            if memo[v] > memo[u] + 1 {
                memo[v] = memo[u] + 1;
                stack.push(v);
        ...
        if let Some(x2) = cols[y].range(x + 1..).next() {
            let v = (x2 - 1, y);
            if memo[v] > memo[u] + 1 {
                memo[v] = memo[u] + 1;
                stack.push(v);
