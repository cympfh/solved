...
fn main() {
    ...
    use Hyper::*;
    let mut dp = ndarray![Inf; 1 << n];
    dp[0] = Real(0_i64);
    for iset in 0_usize..1 << n {
        let i = iset.count_ones() as usize;
        trace!(iset, i);
        for pi in 0..n {
            if iset & (1 << pi) != 0 {
                continue;
            ...
            let cost = dp[iset].unwrap() + (a[pi] - b[i]).abs() * x + f * y;
            dp[iset2] = min!(dp[iset2], Real(cost));
            // trace!((iset, pi), (iset2, cost));
        }
    }
    put!(dp[(1 << n) - 1].unwrap());
}
