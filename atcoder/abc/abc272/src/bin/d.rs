...
fn main() {
    ...
    while let Some((x, y)) = q.pop_front() {
        for &(dx, dy) in d.iter() {
            let (x2, y2) = (x + dx, y + dy);
            if 0 <= x2 && x2 < n as i64 && 0 <= y2 && y2 < n as i64 {
                if memo[(x2, y2)] > memo[(x, y)] + Real(1) {
                    memo[(x2, y2)] = memo[(x, y)] + Real(1);
                    trace!((x2, y2), memo[(x2, y2)]);
                    q.push_back((x2, y2));
                }
            }
        }
