...
fn main() {
    ...
    let n: usize = sc.cin();
    let s: Vec<usize> = sc.chars().into_iter().map(atoi).collect();

    // dp[iset][lastchar] = 何通り
    let mut dp = ndarray![mint!(0); 2048, 11];
    dp[0][10] = mint!(1);
    ...
    for i in 0..n {
        ...
        for iset in 0..2048 {
            ...
            alt[iset][x] = dp[iset][x] * 2;
        }
        // case: last != x
        for iset in 0..2048 {
            for last in 0..11 {
                if last == x || iset & (1 << x) > 0 {
                    continue;
                }
                let jset = iset | (1 << last);
                alt[jset][x] += dp[iset][last];
            }
        }
        dp = alt;
    }
