...
fn main() {
    ...
    let xs: Vec<i64> = sc.vec(q);

    a.sort();
    let asum = Cumsum1d::new(&a);
    trace!(&a);

    for x in xs {
        trace!(x);
        if a[0] > x {
            let ans = asum.sum(0..n) - x * n as i64;
            put!(ans);
            continue;
        }
        if a[n - 1] <= x {
            let ans = x * n as i64 - asum.sum(0..n);
            put!(ans);
            continue;
        }
        let mut left = 0; // a[left] <= x
        let mut right = n;
        for _ in 0..100 {
        ...
        trace!(left, right);
        let a1 = x * (left + 1) as i64 - asum.sum(0..left + 1);
        let a2 = asum.sum(left + 1..n) - x * (n - left - 1) as i64;
        trace!(a1, a2);
        put!(a1 + a2);
    }
}
