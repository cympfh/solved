...
fn score(x: i64, y: i64, modulo: i64) -> i64 {
    let xm = ModInt::new(x, modulo);
    let ym = ModInt::new(y, modulo);
    let a = xm.pow(y) + ym.pow(x);
    a.unwrap()
}
