...
fn main() {
    ...
    let mut dp = ndarray![ModInt::new(0,p); n + 1, 2, n+4];
    dp[1][0][0] = ModInt::new(1, p);
    dp[1][0][1] = ModInt::new(4, p);
    dp[1][1][2] = ModInt::new(3, p);

    for i in 1..n {
        for ex in 0..n {
            let m = dp[i][0][ex] + dp[i][1][ex];
            dp[i + 1][0][ex] += m;
    ...
    let ans: Vec<_> = (1..n).map(|ex| dp[n - 1][0][ex]).collect();
    put!(..ans);
}
