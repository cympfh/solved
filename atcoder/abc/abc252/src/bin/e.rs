...
fn main() {
    ...
    for &(u, v, cost) in edges.iter() {
        g.uedge_with_cost(u, v, Real(cost));
    }
    ...
    memo[0] = Real(0);
    q.push((Reverse(Real(0)), 0, 0));
    while let Some((_cost, u0, u)) = q.pop() {
        if merged[u] {
            continue;
        }
        merged[u] = true;
        used_edges.insert((u0, u));
        for (v, cost) in g.neigh_with_cost(u) {
            if merged[v] {
                continue;
            }
            if memo[v] > memo[u] + cost {
                memo[v] = memo[u] + cost;
                q.push((Reverse(memo[v]), u, v));
            }
        }
    }
    trace!(&used_edges);
    let ans: Vec<usize> = (0..m)
        .filter(|&i| {
            let (u, v, _) = edges[i];
            used_edges.contains(&(u, v)) || used_edges.contains(&(v, u))
        })
        .map(|i| i + 1)
        .collect();
    put!(..ans);
