...
fn main() {
    ...
    // houses
    let mut house_row = DefaultDict::new(BTreeSet::new());
    let mut house_col = DefaultDict::new(BTreeSet::new());
    for _ in 0..n {
        let x: i64 = sc.cin();
        let y: i64 = sc.cin();
        house_row[x].insert(y);
        house_col[y].insert(x);
    }
    let mut ans = 0;
    for _ in 0..m {
        let d: char = sc.cin();
        ...
        let houses = match d {
            'U' => {
                let houses: Vec<(i64, i64)> =
                    house_row[x].range(y..=y + c).map(|&y| (x, y)).collect();
                y += c;
                houses
            }
            'D' => {
                let houses: Vec<(i64, i64)> =
                    house_row[x].range(y - c..=y).map(|&y| (x, y)).collect();
                y -= c;
                houses
            }
            'L' => {
                let houses: Vec<(i64, i64)> =
                    house_col[y].range(x - c..=x).map(|&x| (x, y)).collect();
                x -= c;
                houses
            }
            'R' => {
                let houses: Vec<(i64, i64)> =
                    house_col[y].range(x..=x + c).map(|&x| (x, y)).collect();
                x += c;
                houses
            }
            _ => vec![],
        ...
        ans += houses.len();
        for &(hx, hy) in houses.iter() {
            house_row[hx].remove(&hy);
            house_col[hy].remove(&hx);
        }
    }
    put!(x, y, ans);
}
