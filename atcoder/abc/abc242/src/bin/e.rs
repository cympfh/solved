...
fn main() {
    ...
    for _ in 0..q {
        let n: usize = sc.cin();
        let s: Vec<i64> = sc.chars().into_iter().map(ord).collect();
        let mut dp = ndarray![mint!(0); 3, 3];
        dp[1][1] = mint!(1);

        for i in 0..(n + 1) / 2 {
            trace!(i);
            let mut alt = ndarray![mint!(0); 3, 3];
            for digit in 0..26 {
                for u in 0..3 {
                    let (d1, d2) = pmc(u);
                    let u2 = cmp((d1, digit), (d2, s[i]));
                    for v in 0..3 {
                        let (d1, d2) = pmc(v);
                        let v2 = cmp((digit, d1), (s[n - 1 - i], d2));
                        alt[u2][v2] += dp[u][v];
                    }
                }
            }
        ...
        let ans = dp[0][0] + dp[0][1] + dp[0][2] + dp[1][0] + dp[1][1];
        put!(ans);
    }
}
