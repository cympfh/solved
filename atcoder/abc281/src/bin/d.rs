...
fn main() {
    ...
    // dp[s][m] = ( xs[0..i] のうち s 個の和が m (mod d) )
    let mut dp = ndarray![NegInf; 101, 101];
    dp[0][0] = Real(0);

    for i in 1..=n {
        let mut alt = ndarray![NegInf; 101, 101];
        let x = xs[i - 1];
        for s in 1..=k {
            for m in 0..d {
                let m2 = (m + x) % d;
                alt[s][m2] = max!(alt[s][m2], dp[s - 1][m] + x);
            }
        }
        for s in 0..101 {
            for m in 0..101 {
                alt[s][m] = max!(alt[s][m], dp[s][m]);
            }
        }
        dp = alt;
        trace!(i, x);
