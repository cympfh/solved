...
fn main() {
    ...
    let m = s.len();
    let dots: Vec<i64> = s.iter().map(|&c| if c == '.' { 1 } else { 0 }).collect();
    let cum = Cumsum1d::new(&dots);
    trace!(&dots);
    trace!(&cum);

    let mut ans = 0;
    for base in 0..m {
        if cum.sum(base..base + 1) > k {
            continue;
        }
        if cum.sum(base..m) <= k {
            ans = max!(ans, m - base);
        ...
        let mut left = base + 1; // sum(base..left) <= k
        let mut right = m; // sum(base..right) > k
        for _ in 0..50 {
            let mid = (left + right) / 2;
            if cum.sum(base..mid) <= k {
                left = mid;
            } else {
                right = mid;
            }
        }
        ans = max!(ans, left - base);
    }
    put!(ans);
}
