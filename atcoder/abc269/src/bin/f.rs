...
// 狭義単調増加等差数列の和
fn sum_array(first: ModInt, last: ModInt, num: i64) -> ModInt {
    (first + last) * num / 2
}

// Sum [ f(i, j) | j <- j_left..=j_right ]
fn sum_line(i: i64, j_left: i64, j_right: i64, m: i64) -> ModInt {
    let first = max!(f(i, j_left, m), f(i, j_left + 1, m));
    let last = max!(f(i, j_right, m), f(i, j_right - 1, m));
    let num = (last - first) / 2 + 1;
    sum_array(mint!(first), mint!(last), num)
}
...
fn main() {
    ...
    for _ in 0..q {
        ...
        let c: i64 = sc.cin();
        let d: i64 = sc.cin();
        let first1 = sum_line(a, c, d, m);
        let first2 = sum_line(a + 1, c, d, m);
        let (last1, last2) = {
            let x = sum_line(b, c, d, m);
            let y = sum_line(b - 1, c, d, m);
            if a % 2 == b % 2 {
                (x, y)
            } else {
                (y, x)
        ...
        let num1 = (b - a) / 2 + 1;
        let num2 = b - a + 1 - num1;
        let ans = sum_array(first1, last1, num1) + sum_array(first2, last2, num2);
        put!(ans);
    }
}
