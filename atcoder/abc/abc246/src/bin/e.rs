...
impl D {
    ...
    fn step(&self, u: &(usize, usize), n: usize) -> Option<(usize, usize)> {
        match self {
            LD if u.0 > 0 && u.1 > 0 => Some((u.0 - 1, u.1 - 1)),
            RD if u.0 < n - 1 && u.1 > 0 => Some((u.0 + 1, u.1 - 1)),
            RU if u.0 < n - 1 && u.1 < n - 1 => Some((u.0 + 1, u.1 + 1)),
            LU if u.0 > 0 && u.1 < n - 1 => Some((u.0 - 1, u.1 + 1)),
            _ => None,
        }
    }
}
...
fn main() {
    ...
    let bd: Vec<Vec<char>> = (0..n).map(|_| sc.chars()).collect();
    use Hyper::*;
    let mut dp = ndarray![Inf; n, n, 4];
    dp[s.0][s.1][0] = Real(1);
    dp[s.0][s.1][1] = Real(1);
    dp[s.0][s.1][2] = Real(1);
    dp[s.0][s.1][3] = Real(1);
    ...
    while let Some((u, dir)) = que.pop_front() {
        for &d in [LD, RD, RU, LU].iter() {
            if let Some(v) = d.step(&u, n) {
                let cost = dp[u.0][u.1][dir.value()];
                if bd[v.0][v.1] == '#' {
                    continue;
                }
                if d == dir {
                    if dp[v.0][v.1][d.value()] > cost {
                        dp[v.0][v.1][d.value()] = cost;
                        que.push_front((v, d));
                    }
                } else {
                    if dp[v.0][v.1][d.value()] > cost + 1 {
                        dp[v.0][v.1][d.value()] = cost + 1;
                        que.push_back((v, d));
                    }
                }
            }
    ...
    //     eprintln!();
    // }
    let ans = dp[t.0][t.1].iter().min().unwrap();
    if let Real(ans) = ans {
        put!(ans);
    } else {
        put!(-1);
...
trait Tuple {
    fn scan(sc: &mut Scanner) -> Self;
}
impl<S: FromStr, T: FromStr> Tuple for (S, T) {
    fn scan(sc: &mut Scanner) -> Self {
        let s: S = sc.cin();
        let t: T = sc.cin();
        (s, t)
...
impl<S: FromStr, T: FromStr, U: FromStr> Tuple for (S, T, U) {
    fn scan(sc: &mut Scanner) -> Self {
        let s: S = sc.cin();
        let t: T = sc.cin();
        let u: U = sc.cin();
...
impl Scanner {
    fn tuple<Tup: Tuple>(&mut self) -> Tup {
        Tup::scan(self)
    }
}
