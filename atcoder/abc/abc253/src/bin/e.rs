...
fn main() {
    ...
    // dp[x] = (頭 i 個目まで決めたときに末尾が x で終わる数列の個数)
    let mut dp = vec![mint!(0); m + 1];
    for x in 1..=m {
        dp[x] = mint!(1);
    }
    ...
    for _ in 2..=n {
        trace!(dp);
        let mut alt = vec![mint!(0); m + 1];
        let cumsum = Cumsum1d::new(&dp);
        for x in 1..=m {
            // alt[x] = sum { dp[y] where y <= x-k } + sum { dp[y] where y >= x+k }
            if k > 0 {
                let left = if 1 + k <= x + 1 {
                    cumsum.sum(0..x - k + 1)
                } else {
                ...
                alt[x] = cumsum.sum(0..m + 1);
            }
        }
        dp = alt;
    }
    trace!(dp);
    let ans = dp.into_iter().sum::<ModInt>();
    put!(ans);
}
