...
fn main() {
    ...
    // memo[i][j] = (distance of (i, j), sum-value of inner of (i, j) not containing i & j)
    let mut d = vec![vec![(Inf, Reverse(0_i64)); n]; n];
    for i in 0..n {
        d[i][i] = (Real(0), Reverse(0));
    }
    for u in 0..n {
        for &v in g[u].iter() {
            d[u][v] = (Real(1), Reverse(0));
        }
    }
    for k in 0..n {
        for i in 0..n {
            for j in 0..n {
                let w = d[i][k].0 + d[k][j].0;
                let mut v = d[i][k].1 .0 + d[k][j].1 .0;
                if k != i && k != j {
                    v += a[k];
                }
                let a = (w, Reverse(v));
    ...
    for _ in 0..q {
        let u = sc.usize1();
        let v = sc.usize1();
        if let (Real(w), Reverse(value)) = d[u][v] {
            put!(w, value + a[u] + a[v]);
        } else {
            put!(#Impossible);
        }
