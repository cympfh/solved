...
fn main() {
    ...
    let n: usize = sc.cin();
    let q: usize = sc.cin();
    let arr: Vec<i64> = sc.vec(n); // A

    let mut a = BIT::new(n);
    let mut ai = BIT::new(n); // A * (N-i+1)
    let mut ai2 = BIT::new(n); // A * (N-i+2)**2
    for i in 0..n {
        let d = (n - i) as i64;
        a.assign(i, mint!(arr[i]));
        ai.assign(i, mint!(arr[i]) * d);
        ai2.assign(i, mint!(arr[i]) * d * d);
    }

    for _ in 0..q {
        let ty: usize = sc.cin();
        let i: usize = sc.usize1();
        if ty == 1 {
            let newval: i64 = sc.cin();
            let d = mint!((n - i) as i64);
            a.assign(i, mint!(newval));
            ai.assign(i, mint!(newval) * d);
            ai2.assign(i, mint!(newval) * d * d);
            trace!(&a.debug());
            trace!(&ai.debug());
            trace!(&ai2.debug());
        } else {
            let b = mint!((n - i - 1) as i64);
            trace!(1, 2 * b - 1, b * b - b);
            let ans =
                ai2.sum(0..i + 1) - ai.sum(0..i + 1) * (2 * b - 1) + a.sum(0..i + 1) * (b * b - b);
            put!(ans / 2);
        }
    }
}
...
impl<X: Copy + AGroup> BIT<X> {
    ...
    pub fn assign(&mut self, idx: usize, w: X) {
        self.add(idx, w - self.sum(idx..idx + 1));
    }
    pub fn debug(&self) -> Vec<X> {
        (0..self.size).map(|i| self.sum(i..i + 1)).collect()
    }
    /// sum of [0, idx)
