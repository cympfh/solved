...
fn bitpattern(s: String) -> u32 {
    let mut r = 0;
    for c in s.chars() {
        let i = ((c as u8) - ('a' as u8)) as u32;
        r |= 1 << i;
    }
    r
}
...
fn main() {
    ...
    let mut ans = mint!(0);
    let mut mobius: i64 = 1;
    for choose in 1..=n {
        for v in Combination::new(n, choose) {
            let mut s = ss[v[0]];
            for i in 1..v.len() {
                s &= ss[v[i]];
            }
            let x = mint!(s.count_ones() as i64).pow(len);
            // trace!(choose, s.len(), x);
            ans += x * mobius;
        }
        mobius *= -1;
    }
