...
fn main() {
    ...
    let mut b1 = Binom::new(n - 1, n - 1, 998_244_353);
    let mut b2 = Binom::new(n * n - n, n - 1, 998_244_353);
    let mut b = mint!(0);
    for x in n..=n * n - n + 1 {
        trace!(x);
        b1 = Binom::new_with_hint(x - 1, n - 1, &b1);
        b2 = Binom::new_with_hint(n * n - x, n - 1, &b2);
        trace!(b1, b2);
        b += b1.unwrap() * b2.unwrap();
    }
    trace!(b);
    let f1 = fact(n - 1); //(n-1)!
    let f2 = fact((n - 1) * (n - 1)); //((n-1)^2)!
    trace!(f1, f2);
    let a = b * f1 * f1 * f2 * mint!((n * n) as i64);
    let all = fact(n * n);
    trace!(all, a);
    put!(all - a);
}
